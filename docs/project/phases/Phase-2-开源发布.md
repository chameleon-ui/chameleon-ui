...
> **工程看板（勾选状态）**：[`../../../chameleon-ui/PHASE2.md`](../../../chameleon-ui/PHASE2.md)。本文是目标卡，不重复看板章节。
...
| 0.3 | 公开前清单 | 主题表述（所有者已确认 8 套）、许可证、遥测告知文案草稿就绪 |
...
T2.1–T2.12：扩组件、满 8 主题、21 语、公开 schema、Bench 首期、npm+公网 Registry、`apps/docs`、license/遥测、主题表述复核、门禁走查、盲测计划、回流初值说明。
...
按清单人工点验：性能预算、RTL、伪本地化、license 扫描；官方 8 套主题已所有者确认。红灯不得宣传性发布（npm/公网仍冻结）。
...
本阶段为四大突破点铺基线：**B1 · 开放标准**（公开 schema 为本阶段主战役）；**B2 · RTL-first**（四 RTL）；**B3 · 设计纪律包**（八主题 rules）；**B4 · 数据回流**（Bench + 回流初值）。  
**收口口径**：B1–B4 的最终验收标准与 A1–A6 全六层 DoD 见专项轨道卡 [`AI能力体系-A1-A6-收口轨道.md`](./AI能力体系-A1-A6-收口轨道.md)（含 A1 schema v0.2 键位决议、A5 全量化、A6 `generation_quality` 实测化）。

---

## 9. 讨论会

开源叙事、组件终表、盲测、Bench 指标字典、npm 作用域、T0+90、官方致敬主题以免费 SKU 出货。
