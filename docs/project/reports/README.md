# 阶段验证收报告存放处

每个里程碑退出时在此落一份报告（可按模板从阶段目标文档 §9 复制结构）：

| 里程碑 | 建议文件名 |
| :--- | :--- |
| M0 | `M0-基元POC与地基验收.md` |
| M1 | `M1-MVP验收.md` |
| M2 | `M2-开源发布验收.md` |
| M3 | `M3-v1.0验收.md` |
| M4 | `M4-v2.0建设收口.md` |

当前已落档：

- [`2026-08-15-release-0.1.9.md`](./2026-08-15-release-0.1.9.md)（**0.1.9 发布说明**；源码 zip / npm 未上架说明；Changelog：[`chameleon-ui/CHANGELOG.md`](../../../chameleon-ui/CHANGELOG.md)）
- [`M0-基元POC与地基验收.md`](./M0-基元POC与地基验收.md)
- [`Phase-1-开工检查.md`](./Phase-1-开工检查.md)
- [`Phase-1-主题法务走查.md`](./Phase-1-主题法务走查.md)
- [`M1-MVP验收.md`](./M1-MVP验收.md)
- [`M2-开源发布验收.md`](./M2-开源发布验收.md)
- [`M3-v1.0验收.md`](./M3-v1.0验收.md)
- [`M4-v2.0建设收口.md`](./M4-v2.0建设收口.md)
- [`Phase-4-MCP-Apps决策.md`](./Phase-4-MCP-Apps决策.md)
- [`Phase-4-全量性能与a11y审计.md`](./Phase-4-全量性能与a11y审计.md)
- [`Phase-4-文档21语缺口表.md`](./Phase-4-文档21语缺口表.md)（由 docs 骨架脚本生成）
- [`Phase-5-触控目标测量.md`](./Phase-5-触控目标测量.md)（Token 计算 + 本期清单扫描；无 Lighthouse）
- [`盲测协议.md`](./盲测协议.md)（T9.5 方法；harness `/?view=blind`）
- [`2026-08-15-blind-test-operator-kit.md`](./2026-08-15-blind-test-operator-kit.md)（一页跑法 + `demo:blind` / `blind:validate` / `blind:ingest`）
- [`盲测结果.pending.json`](./盲测结果.pending.json)（`status=not_run`，`rate=null`，`pipelineStatus=ready_for_humans`；禁止手写 %）
- [`schemas/blind-test-result.schema.json`](./schemas/blind-test-result.schema.json)
- [`A9.5-盲测决策-PROTOCOL-READY.md`](./A9.5-盲测决策-PROTOCOL-READY.md) / [`A9.5-decision.json`](./A9.5-decision.json)（0.1.9：非 waive）
- [`Phase-9-VPAT-status.md`](./Phase-9-VPAT-status.md)（published-internal；commercialClaimsAllowed=false；submissionPackReady=true；Not a third-party CAB）
- [`2026-08-15-a11y-cab-submission-pack.md`](./2026-08-15-a11y-cab-submission-pack.md) / [`2026-08-15-a11y-cab-evidence.json`](./2026-08-15-a11y-cab-evidence.json) / [`2026-08-15-a11y-cab-blocker.md`](./2026-08-15-a11y-cab-blocker.md)（CAB 提交包；无假证书）
- [`2026-08-15-theme-quantification.md`](./2026-08-15-theme-quantification.md) / [`.json`](./2026-08-15-theme-quantification.json)（8 主题工程量化：design-rules + S3 gzip；`recognition_rate=null`；≠ 认出率）

移交：[`../handover/建设期移交说明书.md`](../handover/建设期移交说明书.md)。

阶段目标卡见 [`../phases/`](../phases/)。
