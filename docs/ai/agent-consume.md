...
2. CSS: `import "@chameleon-ui/react/css"` (or `@chameleon-ui/vue/css`). Also valid: `@chameleon-ui/themes/line/css` + tokens. Never `.../cupertino/variables.css`.
3. Components: `import { Button, Card, Table } from "@chameleon-ui/react"`. Vue: `import { AppShell, Button, Navigation, ThemeProvider } from "@chameleon-ui/vue"` (catalog 103/103 + ThemeProvider). Underlying `@chameleon-ui/components` / `components-vue` remain valid.
...
6. The consumer app is **not** a pnpm workspace. **NEVER** write `workspace:*`. Install **one** umbrella (`@chameleon-ui/react` or `@chameleon-ui/vue`). Legacy five-pack: see AGENTS.md `--legacy-five`.
7. Disk writes only via `chameleon add` / MCP `install_*` (install-core).
8. SchemaRenderer default map is 10 slugs. React: `@chameleon-ui/schema-renderer`. Vue: `@chameleon-ui/schema-renderer/vue`. For Table / Chart / KpiDashboard import `@chameleon-ui/react` or `@chameleon-ui/vue`.
...
Vue: `import "@chameleon-ui/vue/css"` and JS from `@chameleon-ui/vue`. Slots `#header` / `#navigation`. Template: `templates/external-vite-vue`.
...
Replace both paths. `npx @chameleon-ui/mcp-server` is not available until npm publish (packages are `0.1.9`, still unpublished; use file: / link / pack-external umbrella tarballs).
...
Official Vite + Windows templates: `chameleon-ui/templates/external-vite-react` and `chameleon-ui/templates/external-vite-vue` (one umbrella `file:` dep). Pin React `react@^19` / `@ark-ui/react@5.38.0`, or Vue `vue@^3.5` / `@ark-ui/vue@5.38.1`, plus FormatJS as in AGENTS.md. Height chain: `html, body, #root` (React) or `#app` (Vue) `{ block-size: 100% }`. Dual-track: umbrella package vs `chameleon add` — docs `guides/consume.mdx`. Verify with `pnpm verify:external`.
