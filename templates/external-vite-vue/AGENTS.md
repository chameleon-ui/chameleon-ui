...
3. Root wrap: `<ThemeProvider theme="linear" locale="zh-CN">` (see `src/App.vue`).
...
`AppShell` + `Navigation` + `NavigationTitle` + `useTabStacks`. Slots `#header` / `#navigation`. Do not compose `Sidebar` + `TabBar`.
