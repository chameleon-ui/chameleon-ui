...
AG-UI 协议适配（**POC** 级，Phase 8 新建）。L3/L4 包，不进 L1/L2。

## 支持级别

**POC（决策记录见 [DECISION.md](./DECISION.md)）**：双向状态同步已实现且有断言测试；尚未与 AG-UI 参考实现做线上互操作认证，不宣称协议认证。版本承诺：POC 阶段 `0.0.x`，事件形状变更将在 CHANGELOG 记录。
...
- 断线恢复：`disconnect()` 后事件丢弃，`reconnect()` 由 agent 发起 `SYNC_REQUEST`，frontend 全量重同步。
...
打印一次「用户输入 → agent delta 应答 → 断线重连重同步」的完整事件转录（[demo/state-sync.mjs](./demo/state-sync.mjs)）。
