...
describe('MDX component pages (contract SSOT)', () => {
  it('renders the full structured template for a gold-batch component', () => {
    ...
    expect(document.querySelector('[data-docs="composition"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="data-ai"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="mechanics"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="events"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="agent"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="exports"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="events"]')?.textContent).toMatch(/onClick/)
  ...
  it('documents Navigation API, events, stacks, and agent emit rules', () => {
    const { t } = getTranslator('en')
    render(<ComponentPage slug="navigation" t={t} />)
    expect(document.querySelector('[data-docs="mechanics"]')).not.toBeNull()
    expect(document.querySelector('[data-docs="events"]')?.textContent).toMatch(/onSelect/)
    expect(document.querySelector('[data-docs="events"]')?.textContent).toMatch(/onCollapsedChange/)
    expect(document.querySelector('[data-docs="exports"]')?.textContent).toMatch(/splitNavigationItems/)
    expect(document.querySelector('[data-docs="agent"]')?.textContent).toMatch(/get_contract/)
    expect(document.querySelector('[data-docs-preview="navigation"]')).not.toBeNull()
  })

  it('documents NavigationBar useTabStacks as a public export', () => {
    const { t } = getTranslator('en')
    render(<ComponentPage slug="navigation-bar" t={t} />)
    expect(document.querySelector('[data-docs="exports"]')?.textContent).toMatch(/useTabStacks/)
    expect(document.querySelector('[data-docs="events"]')?.textContent).toMatch(/onBack/)
    expect(document.querySelector('[data-docs="agent"]')?.textContent).toMatch(/get_contract/)
  })
})
...
describe('docs public artifacts', () => {
  ...
  it('publishes an agent integration spec and a three-end chrome guide', async () => {
    const agents = await readFile(path.join(here, '../docs/guides/for-agents.mdx'), 'utf8')
    ...
    expect(threeEnd).toContain('Navigation')
    expect(threeEnd).toContain('NavigationBar')
    expect(threeEnd).toContain('ActionSheet')
    expect(threeEnd).toContain('density.css')
    expect(threeEnd).toContain('TabBar')
    expect(threeEnd).not.toContain('容器查询实证')
  })
})
