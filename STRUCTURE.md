...
| `registry` | `@chameleon-ui/registry` | 目录；CLI/MCP/Bench 只读；不写盘。无 `CU_REGISTRY_URL` 时用 bundled；有 URL 则 HTTP 客户端；含 `registry:block`（`registry/b/`） |
...
| `blocks` | `@chameleon-ui/blocks` | Phase 7 场景 Blocks ×12；`manifest.json` + `registry:block`；安装仅经 install-core；矩阵 `scenario-matrix.json` |
| `schema-renderer` | `@chameleon-ui/schema-renderer` | Phase 8 运行时 JSON→组件树；默认 map 10 slug |
| `adapter-ag-ui` | `@chameleon-ui/adapter-ag-ui` | Phase 8 AG-UI **POC**（非协议认证）；决策树 → slug → install-core |
...
| `cli` | `@chameleon-ui/cli` | 薄壳 `chameleon`；只调 `install-core`；可选 `CU_REGISTRY_URL` / `CU_REGISTRY_TOKEN`；含 `add-block` |
| `mcp-server` | `@chameleon-ui/mcp-server` | 薄壳 MCP；工具名 snake_case；只调 `install-core`；同一套 `CU_REGISTRY_*`；只读工具 `get_contract` / `get_design_rules` / `get_import_specifiers`；写盘含 `install_block` |
...
| 路径 | npm 名 | 注解 |
| :--- | :--- | :--- |
| `apps/internal-demo` | `@chameleon-ui/internal-demo` | T1.10 内测 Demo。**不是**公开文档站。选择器：21 Locale + 8 主题。三端一体：`/?view=three-end`（390/768/1280 iframe）。盲测：`/?view=blind`。Blocks：`/?view=blocks`（P7 十二场景；协议在 `docs/project/reports/盲测协议.md`；结果 pending，禁止手写认出率） |
...
> 原延期表中的 `blocks` / `schema-renderer` / `adapter-ag-ui` 已迁入「当前 packages/」。无新增空包占位。
