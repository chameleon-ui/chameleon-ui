...
export interface ToastProviderProps {
  ...
  closeLabel?: string
  /**
   * When true, wraps children in a `block-size: 100%` host so AppShell's
   * percentage height resolves even though ToastProvider also mounts Toaster
   * as a sibling. Prefer this over consumer `[data-cu-shell]` patches.
   */
  fill?: boolean
}
...
export function ToastProvider({
  ...
  const body = fill ? <div className="cu-toast-provider__fill">{children}</div> : children
