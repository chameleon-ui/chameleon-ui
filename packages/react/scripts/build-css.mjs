/**
 * Emit a real CSS artifact for `@chameleon-ui/react/css`.
 *
 ...
 * theme + components base styles into dist/css.css. Also copy each theme
 * overlay into dist/themes/<id>.css for `@chameleon-ui/react/themes/<id>/css`.
 *
 * Run after dependent packages are built (tokens, themes, components).
 */
...
function resolveCss(specifier) {
  try {
    ...
    throw new Error(
      `[@chameleon-ui/react] cannot resolve ${specifier} (${reason}). Build tokens/themes/components first.`,
    )
  }
}
...
const layers = [
  ...
  { label: '@chameleon-ui/components-react/css', specifier: '@chameleon-ui/components-react/css' },
]
...
for (const layer of layers) {
  ...
  if (!css.trim()) {
    throw new Error(`[@chameleon-ui/react] empty CSS from ${layer.specifier}`)
  }
...
await writeFile(
  ...
  '/** Side-effect CSS entry: tokens + density + line + components. */\nexport {}\n',
)
...
console.log(
  `[@chameleon-ui/react] wrote dist/css.css (${bytes} bytes) + dist/themes/{${THEME_IDS.length} ids}.css`,
)
