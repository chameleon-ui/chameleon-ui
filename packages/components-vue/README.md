...
**L2 · Vue 组件实现（catalog 116/116 + ThemeProvider）。**

Vue 侧的完整组件实现，与 React 主包（@chameleon-ui/components-react）**catalog 对齐 116/116**。React 包仍是契约 SSOT；本包按同契约移植。

## 覆盖

- Catalog Vue 覆盖 **116/116**；`ThemeProvider` 是额外出口（非 catalog slug）。
...
- **CSS**：消费者主题用 `@chameleon-ui/themes/<id>/css`（默认 `linear`）。包 CSS 是 barrel 与 `@chameleon-ui/components-vue/css` 的副作用。
...
import { AppShell, Button, Navigation, NavigationTitle, ThemeProvider } from '@chameleon-ui/components-vue'
